...
!!! note "推奨されるアセット構造"
    ...
    この設計により、**プリムの名前や階層構造が変わらない範囲の CAD 更新**（メッシュ形状やマテリアルの差し替えなど）であれば、Asset Source を差し替えるだけで済み、リギングや物理設定を作り直す必要がなくなります。ただし、上位レイヤーの物理・リギング設定は特定のプリムパスを参照しているため、CAD 側でリンクの追加・削除や名前変更など**構造が変わる更新**をした場合は、対応する設定の修正が必要になる点に注意してください。
...
!!! tip "Layer タブの作業が終わったら Stage パネルに戻る"
    サブレイヤーの挿入が終わったら、以降のリペアレントやプリム作成は **Stage** パネルから行います。Layer タブはレイヤーの管理や編集ターゲットの切り替え専用で、プリムを直接選択・編集する場所ではありません。Layer タブの中でプリムを探そうとすると迷子になるので注意してください。
