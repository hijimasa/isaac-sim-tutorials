...
!!! tip "チェックポイントアセットからの開始"
    前回のチュートリアルを完了していない場合は、画面左下の **Content** タブから **Samples > Rigging > MockRobot** フォルダ内の `mock_robot_rigged` アセットをロードすることで、このチュートリアルから開始できます。ロード時は **File（参照ではなくファイルとして）** で開いてください。
