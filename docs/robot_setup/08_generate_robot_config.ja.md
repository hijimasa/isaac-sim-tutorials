...
| **Lula 用アセット** | `Samples > Rigging > Manipulator > configure_manipulator > ur10e > ur > ur_gripper_lula.usd` | Instantiable 解除済みアセット（ステップ 2 で使用） |
...
3. **Property** パネルで **Instantiable** フィールドのチェックを外します。

    ![Instantiable メッシュの解除](images/42_disable_instantiable_mesh.png)

    !!! tip "Instantiable が見つからないときは？"
        選択されているメッシュに**Instantiable**が有効なものと無効なものが混在している可能性があるので、注意深く選択して混在を防ぐ必要があります。
...
2. **Lula Robot Description Editor** のセットアップとアセットの準備（Instantiable の解除）
