...
!!! note "Isaac Lab と Isaac Sim の役割分担"
    **Isaac Lab** は Isaac Sim を基盤とするロボット学習フレームワークで、強化学習・模倣学習のための API とサンプル環境を提供します。典型的なワークフローは次のとおりです：
...
| `initialize` | シミュレーション開始後に一度だけ呼ぶ。エフォートモード、制御モード、ジョイントゲイン、最大トルク・最大速度、アーティキュレーションルートをポリシーに合わせて設定する |
...
ANYmal Flat Terrain Policy クラスの `initialize` では、LSTM ベースの SEA ネットワーク（`LstmSeaNetwork`）にポリシーファイルを読み込ませています：
