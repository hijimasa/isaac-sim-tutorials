...
ROS 2 ノード経由で URDF をインポートすると、既存の ROS 2 ワークフローと Isaac Sim を直接連携できます。`robot_state_publisher` が配信するロボット記述を読み込むため、**XACRO ファイルも明示的に URDF へ変換することなく間接的にインポートできる**のが大きな利点です。
