...
!!! warning "チュートリアル完了後の GPU Dynamics 設定について"
    このチュートリアルでは GPU Dynamics を有効にしましたが、他のチュートリアル（剛体やアーティキュレーションを扱うもの）では GPU Dynamics が有効だと `setDriveTarget()` などの API が使用できず、エラーが発生する場合があります。このチュートリアルの完了後、次のチュートリアルに進む前に **Physics Scene** の **Enable GPU Dynamics** をオフに戻してください。
