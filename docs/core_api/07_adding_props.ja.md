...
!!! note "Xform と Mesh の違い"
    ステージ上のルービックキューブは、**Xform**（位置・回転・スケールを持ち、子プリムをまとめる「入れ物」のプリム）の下に、実際の形状データである **Mesh** プリムがぶら下がる階層構造になっています。物理属性は、オブジェクト全体を代表する **Xform 側に付与**します。Stage パネルの Type 列（Xform / Mesh）で両者を見分けられます。
