...
**方法 1：carb 設定を変更する** — シミュレーションのタイムラインの実行レートを設定します。**On Playback Tick** ノード由来の時間に影響します。シーンを再生した後に実行してください：
...
- CPU 使用率を確認してボトルネックを特定してください。Isaac Sim の使用率が非常に高い場合は、Fabric を有効にした起動を試せます：
