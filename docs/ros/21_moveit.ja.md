...
!!! warning "ROS 2 Humble での既知の問題"
    この MoveIt 2 チュートリアルは、ROS 2 Humble ではパフォーマンス上の制約により、プランニングや実行が断続的に失敗することがあります。失敗やタイムアウトが頻発する場合は、この問題が発生しない **ROS 2 Jazzy** への切り替えを検討してください。

!!! warning "Windows（Pixi）非対応"
    このチュートリアルは Windows の Pixi ベースのインストールではサポートされていません。Linux の ROS 2 環境を使用してください。
...
!!! note "Humble で断続的に失敗する場合"
    ROS 2 Humble では、パフォーマンス上の問題によりプランニングや実行の途中でチュートリアルが断続的に失敗することがあります。これは既知の制限で、現時点で完全な修正はありません。失敗が繰り返される場合は ROS 2 Jazzy への切り替えを推奨します。
